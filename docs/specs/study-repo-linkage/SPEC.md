...
- **Dates excluded from derivation are an explicit SHA list, never a file-count heuristic.** The two populations overlap in both directions: mechanical commits run from 1 note file to 120, and the largest genuine study commits, at 51 and 38, sit inside that range. Any threshold rule discards real study dates. See [migration-plan.md](migration-plan.md).
